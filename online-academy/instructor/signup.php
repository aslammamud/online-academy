		...
		if($password1==$password2){
			...
			if(!empty($password)){
    			...
    			if($data['ins_email'] == $email){
    			 ...
    			 $insertuser = "INSERT INTO `instructor`(`ins_name`, `ins_pass`, `ins_phone`, `ins_email`, `ins_sex`, `ins_nid`, `ins_dob`, `ins_education`, `ins_joining_date`, `ins_facebook`, `ins_address`, `ins_verified`) 
            	 VALUES ('$name', '$password', '$phone', '$email', '$sex', '$nid', '$dob', '$education', '$join_date', '$facebook', '$address', 0)";
                	 ...
                	 if (mysqli_query($con, $insertuser)) {
                        ...
                        $ins_savings_acc_sql = "INSERT INTO `instructor_savings_account`(`ins_id`, `sav_payable`, `sav_total`, `sav_trasc_date`) VALUES ('$instruct_id',0,0, '$join_date')";
                        $res_sav_acc = mysqli_query($con,$ins_savings_acc_sql);
		   ...
		   <div class="contacta" style="/*background:url(\'https://i.pinimg.com/originals/3d/08/e0/3d08e03cb40252526fee2036a67f07f1.gif\') no-repeat center; background-size:850px 500px; border-radius: 25px;">
			...
			<div class="texth mt-4">সাইন-আপ সম্পন্ন করতে প্রয়োজনীয় তথ্য দিন</div>
					...
					<form class="apply_sc" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) ?>" method="POST">
						    ...
						    <br><input class="inptex" type="date" name="dob" placeholder="আপনার  জন্ম  তারিখ" value="" required>							
