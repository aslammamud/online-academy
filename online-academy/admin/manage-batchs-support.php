...
//require('super-admin-check.php');
		...
		if($di_stf_desiganation == 8){}else{die();}
...
while ($row = mysqli_fetch_array($query)) {
	echo '<tr> 
		 ...
		 echo '<td>';
		  ...
		  echo '
<a  href="'.$site_link.'/admin/show-batch-students.php?batch='.$row ['b_name'].'&course='.$row ['oc_id'].'&cmd=paid" class="badge badge-success" >Show Paid</a><br>
<a  href="'.$site_link.'/admin/show-batch-students.php?batch='.$row ['b_name'].'&course='.$row ['oc_id'].'&cmd=unpaid" class="badge badge-info" >Show UnPaid</a><br>
</td> 
		  </tr> ';
}
